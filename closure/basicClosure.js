...
// Not using closures
const signMessage = (message, salutation, name) =>
  `${message}\n\n${salutation},\n${name}\n\n`;
...
if (require.main === module) {
  ...
  const friendlyMessageFromAnimal = signAsAnimal(
    "Hey folks! Party at my place tonight, bring snacks!",
    "See ya soon"
  );
  console.log(friendlyMessageFromAnimal);

  const signForBusiness = signWith("Best");
  const officeMessage = signForBusiness(
    "Hi, Peter. We need to talk about your TPS reports.",
    "Bob"
  );
  console.log(officeMessage);

  const shakespeareanSign = makeSignature("Thine evermore", "William");
  const sonnet = shakespeareanSign(
